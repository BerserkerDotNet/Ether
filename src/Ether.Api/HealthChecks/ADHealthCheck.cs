...
namespace Ether.Api.HealthChecks
{
    public class ADHealthCheck : IHealthCheck
    {
        ...
        public Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            ...
            if (config is null || string.IsNullOrEmpty(config.Type))
            {
            ...
            return Task.FromResult(HealthCheckResult.Healthy("AD config is OK.", data: new Dictionary<string, object> { { "CurrentValue", config } }));
        }
    }
}
